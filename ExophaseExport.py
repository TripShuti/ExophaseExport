...
def classify_platform(platforms: list[dict]) -> str:
    """Return tab name based on platform list."""
...
def build_row(game: dict, platform: str) -> tuple:
    """Build a DataTable row tuple for the given platform."""
...
class FilterModal(ModalScreen[str | None]):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "apply":
            ...
            self.dismiss("")   # empty string = clear filter
        else:
            self.dismiss(None)
...
class ExophaseExport(App):
    ...
    def load_data(self, filter_query: str = "") -> None:
        """Reload all tables, optionally filtering by title substring."""
    ...
    def action_delete_game(self) -> None:
        ...
        # Find by title + platform-specific fields for safer matching
    ...
    def export_csv_data(self) -> None:
        """Export the currently active tab as a flat CSV."""
        if not self.games_data:
        ...
        active_id = tabs.active                              # e.g. "tp-steam"
